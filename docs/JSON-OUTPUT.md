  ...
  "results": [
    {
      ...
      "is_hvd": false,
      "link": "https://data.europa.eu/data/datasets/...",
      "distributions": [
        { "format": "CSV", "url": "https://..." }
      ]
    }
  ],
  "facets": {
    "country": [
      { "id": "de", "title": "Germany", "count": 657 }
    ],
    "format": [
      { "id": "CSV", "title": "CSV", "count": 123 }
    ],
    "categories": [
      { "id": "SOCI", "title": "Population and society", "count": 45 }
    ]
  }
}
```
...
|-------|------|-------|
| `count` | number | Total matching datasets |
| `results[].id` | string | Dataset URI |
| `results[].title` | string | Title in requested language |
| `results[].description` | string\|null | Max 200 chars |
| `results[].issued` | string\|null | ISO timestamp |
| `results[].modified` | string\|null | ISO timestamp |
| `results[].country` | string\|null | ISO 3166-1 alpha-2 uppercase |
| `results[].formats` | string[] | Unique format labels |
| `results[].keywords` | string[] | Max 5 keywords |
| `results[].is_hvd` | boolean | High Value Dataset flag |
| `results[].link` | string | Portal URL |
| `results[].distributions` | array | Max 3 distributions (`format` + `url`) |
| `facets` | object | Keyed by facet id (country, format, categories, etc.) |
| `facets.*[]` | array | Top 15 items sorted by count desc |
| `facets.*[].id` | string | Facet value identifier |
| `facets.*[].title` | string | Human-readable label |
| `facets.*[].count` | number | Number of matching datasets |

**Available facet keys**: `country`, `format`, `categories`, `catalog`, `publisher`, `license`, `is_hvd`, `hvdCategory`, `keywords`, `language`, `type`, `accessRight`, `contactPoint`, `scoring`, `subject`, `superCatalog`, `conformance`, `dataScope`.
